...
- Root cause A — SDL core not initialized (fixed in v0.2.91): `ArInit` was calling `SDL_InitSubSystem(SDL_INIT_AUDIO)` which requires SDL core to already be running. HydraHeadiPad bypasses the Moonlight/SDL UIKit main loop so SDL core was never initialized, causing this failure on every stream attempt. Symptom: `ArInit: SDL_Init(audio) failed: Application didn't initialize properly...`. Fixed by switching to `SDL_Init(SDL_INIT_AUDIO)` which brings up the core first if needed.
...
- Reading the log: check the `ArInit:` entry for `channels=N` and the SDL error line immediately after. channels=2 + `SDL_Init(audio) failed` → root cause A (update to v0.2.91). channels > 2 + open failure → root cause B (Sunshine surround config issue, stereo fallback should recover).
