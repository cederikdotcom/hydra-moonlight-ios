...
- Fixed (318662e): `ArInit` now sets `AVAudioSessionCategoryPlayAndRecord` with `DefaultToSpeaker | MixWithOthers`. This allows concurrent mic capture and speaker playback with no duck-others side-effect.
- If mic relay stops working after an upstream rebase: check `Connection.m` around the `SDL_PauseAudioDevice` call — the `setCategory` call must use `AVAudioSessionCategoryPlayAndRecord`, not `AVAudioSessionCategoryPlayback`.
