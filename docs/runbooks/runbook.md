...
| `Limelight/Input/NativeTouchHandler.h/.m` | Single-finger touch → absolute mouse simulation (`LiSendMousePositionEvent` + `LiSendMouseButtonEvent`). Second finger during a press cancels the active click. Two-finger pinch handled by `UIPinchGestureRecognizer` on `StreamView`. |
...
**Microphone relay (hydravoie) has no effect — Mercator (or any voice experience) cannot hear the user**
- Symptom: iPad mic permission is granted, `MicRelay: started` appears in logs, RTP packets are sent, but the body's hydravoice receives nothing usable.
- Root cause: `ArInit` in `Connection.m` called `setCategory:AVAudioSessionCategoryPlayback`. `Playback` does not allow audio input. Even if the mic relay set `PlayAndRecord` earlier, `ArInit` runs after the stream connects (several seconds later), overriding the category back to input-blocking `Playback`. The mic tap kept running but `AVAudioEngine` silently discarded all input because the session no longer allowed it.
- Fixed (318662e): `ArInit` now sets `AVAudioSessionCategoryPlayAndRecord` with `MixWithOthers`, followed by `overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker`. This allows concurrent mic capture and speaker playback with no duck-others side-effect.
- Updated (5b8005a): `ArInit` now checks `currentRoute.outputs` at startup. If an external output (USB, headphones, Bluetooth) is connected it passes `AVAudioSessionPortOverrideNone` so the session routes to the connected device. If no external device is present it passes `AVAudioSessionPortOverrideSpeaker` so audio plays from the iPad's built-in speakers. USB-C EarPods and Lightning EarPods appear as `AVAudioSessionPortUSBAudio` and were silently ignored by the unconditional `PortOverrideSpeaker`; the conditional check fixes that. Log line: `ArInit: audio output override=none|speaker (hasExternalOutput=1|0)`.
- If mic relay stops working after an upstream rebase: check `Connection.m` around the `SDL_PauseAudioDevice` call — the `setCategory` call must use `AVAudioSessionCategoryPlayAndRecord` with `MixWithOthers`, not `AVAudioSessionCategoryPlayback`.
- If mic relay stops working after an upstream rebase: check `Connection.m` around the `SDL_PauseAudioDevice` call — the `setCategory` call must use `AVAudioSessionCategoryPlayAndRecord` with `MixWithOthers`, not `AVAudioSessionCategoryPlayback`.
- Prerequisite: the experience must have `enable_microphone: true` in its HydraCluster config, and the iPad user must have granted microphone permission (Settings → Privacy → Microphone → HydraHeadiPad). The app only requests permission the first time `micRelay.start` runs; if previously denied the relay silently does nothing — direct the user to grant it in Settings.
...
- Root cause A — SDL not initialized (fixed in v0.2.92): On iOS, SDL2 internally requires `SDL_UIKitRunApp` to set an `SDL_application_is_initialized` flag before any `SDL_Init` or `SDL_InitSubSystem` call will succeed. HydraHeadiPad uses its own Swift/UIKit main and never calls `SDL_UIKitRunApp`, so both `SDL_InitSubSystem` (v0.2.90) and `SDL_Init` (v0.2.91) failed with "Application didn't initialize properly". Fixed in v0.2.92 by calling `SDL_SetMainReady()` — the documented SDL API for apps that bypass SDL_main — immediately before `SDL_Init(SDL_INIT_AUDIO)` in `ArInit`. `#define SDL_MAIN_HANDLED` (already present in Connection.m) makes the symbol available without including SDL_main.h.
- Root cause B — surround-sound channel mismatch (v0.2.90 stereo fallback): iOS audio hardware only supports stereo output. If Sunshine sends a surround-sound Opus config (channelCount > `IOS_MAX_AUDIO_CHANNELS` = 2), `SDL_OpenAudioDevice` fails. Since v0.2.90 `ArInit` retries with stereo before giving up. Symptom: `ArInit: SDL_OpenAudioDevice failed with N channels — retrying with 2 (stereo fallback)`.
...
Our additions are in `HydraStreamSession.*`, `HydraPairSession.*`, `HydraOpus.*`, and `Limelight/Input/NativeTouchHandler.*`. `StreamView.h/.m` (touch routing, pinch gesture, `getVideoAreaSize`, `adjustCoordinatesForVideoArea:`) and `StreamFrameViewController.m` are also lightly patched — check these files when rebasing.
