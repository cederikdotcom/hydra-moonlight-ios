...
- Fixed (318662e): `ArInit` now sets `AVAudioSessionCategoryPlayAndRecord` with `MixWithOthers`, followed by `overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker`. This allows concurrent mic capture and speaker playback with no duck-others side-effect, and correctly routes audio to the built-in speaker when no external device is connected while deferring to an external device when one is present.
- `DefaultToSpeaker` must NOT be used — it forces the built-in speaker even when an external USB or headphone device is connected, silencing it. Use `overrideOutputAudioPort` instead (36dcff3).
